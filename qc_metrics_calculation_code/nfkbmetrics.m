function [metrics, sc_metrics, aux, graph, info, measure] = nfkbmetrics(expt)
%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
...
% sc_metrics    structure with only metrics needed for signaling codon
%               calculations (line 520)
...
%% COLLECT METRICS NEEDED FOR SIGNALING CODON CALCULATIONS
% Duration
sc_metrics.dur_t = metrics.duration(:, 2);
sc_metrics.n_pks = metrics.num_peaks;
%EarlyVsLate
sc_metrics.eVl = metrics.time2HalfMaxIntegral;
%OscVsNon
sc_metrics.oVn = metrics.oscpower;
%PeakAmplitude
sc_metrics.max_val = metrics.max_value;
sc_metrics.pk2pk = metrics.peak2peak;
sc_metrics.pk1_amp = metrics.pk1_amp;
%Speed 
sc_metrics.max_pk1_spd = metrics.max_pk1_speed;
sc_metrics.pk1_t = metrics.pk1_time;
sc_metrics.deriv2 = metrics.derivatives(:, 2);
%TotalActivity
sc_metrics.tot_act = metrics.max_pos_integral;
end